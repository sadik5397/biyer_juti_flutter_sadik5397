...
class RequestButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(children: [
      Expanded(
        child: Container(
            ...
            decoration: BoxDecoration(
                ...
                gradient: state == RequestState.requestContact ? ThemeGradient.blueBlack : null,
                color: state == RequestState.requestPending
                    ? HexColor("#EBEDF0")
      ...
      if (state == RequestState.contactInfo) FloatingActionButton(shape: const CircleBorder(), onPressed: () async => await launchUrl(Uri.parse("tel:$mobileNumber")), mini: true, elevation: 5, backgroundColor: ThemeColor.primary, child: const Icon(FeatherIcons.phone, size: 16, color: Colors.white)),
      if (state == RequestState.contactInfo)
        FloatingActionButton(shape: const CircleBorder(), onPressed: () async => await Clipboard.setData(ClipboardData(text: mobileNumber.toString())), mini: true, elevation: 5, backgroundColor: ThemeColor.primary, child: const Icon(FeatherIcons.copy, size: 18, color: Colors.white)),
    ]);
  }
}
